    ...
    <v-flex xs12>
      <v-list>
        <v-toolbar color="white" flat fixed>
          <v-btn href="/student/driver" icon light>
            <v-icon color="grey darken-2">arrow_back</v-icon>
          </v-btn>
          <v-toolbar-title class="title font-weight-thin">Bus No.: {{ student.bus_id }}</v-toolbar-title>
        </v-toolbar>
...
export default {
  ...
  created() {
    ...
    Echo.channel("messages-sent."+this.student.bus_id).listen( "SentMessage",
      e => {
        this.allMessages.push({ driver: e.message.driver, driver_message: e.message.driver_message, created_at: e.message.created_at });
  ...
  methods: {
    ...
    sendMessages() {
      axios.post('/student/driver/message', { message: this.message })
      .then(response => {
        this.allMessages.push({ student: response.data.message.student, student_message: response.data.message.student_message, created_at: response.data.message.created_at });
        setTimeout(this.scrollToEnd, 100);
      });
      this.clearMessage();
    },
    ...
    fetchMessages() {
      axios.get('/student/driver/message/recieve')
      .then(response => {
