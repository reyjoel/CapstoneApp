...
<div class="yellow accent-4">
    <!-- BACK ARROW ICON -->
      ...
      <v-btn style="text-decoration:none;" href="/student/home" icon light>
   ...
   <!-- PROFILE ICON -->
    ...
    <!-- PROFILE NAME -->
            <p class="title font-weight-light py-3" >{{ student.fname }} {{ student.lname }}</p>
        </div>
</div>
    <!-- PROFILE INFO -->
        <div class="text-xs-center subheader font-weight-light py-3">
            <p class="title font-weight-thin mb-4">Student No.: {{ student.stu_id }}</p>
            <p class="title font-weight-thin mb-4">Email Address: {{ student.email }}</p>
        </div>

  <div class="text-xs-center mt-1">
      <!--Edit Info -->
        <v-btn v-on:click="clickStudent(student)" color="transparent" depressed style="font-size: 10px" data-toggle="modal" data-target="#editinfo">Edit Info</v-btn>
      ...
      <!--Change Email -->
        <v-btn v-on:click="clickStudent(student)" color="transparent" depressed style="font-size: 10px" data-toggle="modal" data-target="#changeemail">Change Email</v-btn>
...
export default {
  ...
  methods: {
    ...
    return (this.fname = student.fname, this.lname = student.lname, this.email = student.email);
    },

    updateInfo() {
   axios
   .put("/student/updateinfo", {fname: this.fname, lname: this.lname, email: this.email})
   ...
   .catch(error => {
    ...
    updateEmail() {
   axios
   .put("/student/updateemail", {email: this.email})
   .then(response => {window.location.reload()})
   .catch(error => {
    ...
    updatePassword() {
   axios
   .put("/student/updatepassword", {password: this.password, password_confirmation: this.password_confirmation})
   .then(response => {window.location.reload()})
   .catch(error => {
