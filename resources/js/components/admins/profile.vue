  ...
  <div>
    ...
    <div class="blue-grey lighten-5">
      ...
      <div class="text-xs-center">
        ...
        <!-- PROFILE NAME -->
        <p class="title font-weight-light py-3">{{ admin.fname }} {{ admin.lname }}</p>
      </div>
    </div>
    <!-- PROFILE INFO -->
    <div class="text-xs-center subheader font-weight-light py-3">
      <p class="title font-weight-thin">Email Address: {{ admin.email }}</p>
    </div>

    <div class="text-xs-center mt-1">
      <!--Edit Info -->
        <v-btn v-on:click="clickAdmin(admin)" color="transparent" depressed style="font-size: 10px" data-toggle="modal" data-target="#editinfo">Edit Info</v-btn>
      ...
      <!--Change Email -->
        <v-btn v-on:click="clickAdmin(admin)" color="transparent" depressed style="font-size: 10px" data-toggle="modal" data-target="#changeemail">Change Email</v-btn>
...
export default {
  ...
  methods: {
    ...
    return (this.fname = admin.fname, this.lname = admin.lname, this.email = admin.email);
    },

    updateInfo() {
   axios
   .put("/admin/updateinfo", {fname: this.fname, lname: this.lname, email: this.email})
   ...
   .catch(error => {
    ...
    updateEmail() {
   axios
   .put("/admin/updateemail", {email: this.email})
   .then(response => {window.location.reload()})
   .catch(error => {
    ...
    updatePassword() {
   axios
   .put("/admin/updatepassword", {password: this.password, password_confirmation: this.password_confirmation})
   .then(response => {window.location.reload()})
   .catch(error => {
